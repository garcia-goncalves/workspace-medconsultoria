...
- **E-mail dentro da aplicação — Bloco 1: plugar e ler (ADR-95, 2026-08):** cada pessoa pluga a própria caixa IMAP em **`/email`** (senha do webmail, testada antes de gravar e guardada cifrada por `EMAIL_CRYPTO_KEY`) e lê sem sair do Workspace — 3 colunas, corpo em `iframe sandbox=""`, imagem remota bloqueada. **Índice + cache** (corpo só ao abrir; busca por `ESEARCH` no servidor); sincronização incremental por `QRESYNC`/`CONDSTORE`; **sem `IDLE`** (o lsnode derruba o Node ocioso — mesma causa do ADR-84). Regra: **a caixa é privada, a correspondência com o cliente é da empresa**. Só `@medconsultoria.com.br` nesta fase.
- **E-mail dentro da aplicação — Bloco 2, fase 2A: escrever, responder, encaminhar, anexar, rascunho (ADR-96, 2026-08):** `/email` passou a **enviar** por SMTP real da caixa da pessoa — escrever do zero, responder, responder a todos, encaminhar (preenche `References`, não marca `\Answered` — encaminhar não é responder) e anexar (subir até 20 MB por arquivo/25 MB somados por envio, anexos novos e do original no mesmo teto; **o anexo recebido é um link que baixa** em stream com `Content-Disposition: attachment` + `X-Content-Type-Options: nosniff`, nunca renderiza — a rota existia desde o começo, mas só em 05/08 a tela passou a chamá-la). **Encaminhar leva os anexos do e-mail original** (o navegador manda só os ids; quem rebaixa do IMAP é o servidor). **Rascunho grava sozinho na pasta `Drafts` do servidor** a cada 5 s (reverte o ADR-95 §2: agora aparece no webmail e em qualquer outro cliente de e-mail), regravando por cima do anterior e apagando depois do envio. Assinatura entra só no servidor (nunca duplicada); a citação tem **duas versões**: a da **tela** sempre bloqueia imagem remota (para não disparar pixel de rastreio enquanto a pessoa ainda escreve) e a de **envio** só restaura imagem em **resposta** — quem recebe de volta é a mesma pessoa que mandou o original; **encaminhamento sai bloqueado**, porque quem recebe é um terceiro que nunca escolheu abrir aquele e-mail (ADR-96 §5). Nota honesta: como o corpo é gravado no banco já sanitizado com a imagem bloqueada, hoje nem na resposta há `src` para restaurar — o contrato existe para o dia em que o corpo chegar cru por outro caminho. Trava de destinatário fora de produção (`conferirDestinoPermitido`) recusa qualquer e-mail que não seja um dos dois de teste do dono, porque o SMTP é real em qualquer ambiente.
- **E-mail dentro da aplicação — Bloco 2, fase 2D‑1: a conversa com o cliente na ficha (ADR-97, 2026-08):** o card **"E-mails"** da ficha do cliente (e do painel do lead) deixou de mostrar só o log dos disparos automáticos: agora junta a esse log **o que a equipe trocou pela própria caixa**, com **selo por origem** ("Enviado pelo sistema" × "Caixa de \<pessoa\>"). A equipe lê **remetente, destinatários, assunto, data e trecho — nunca o corpo** (o corpo é do dono, em `/email`): ampliar depois é um `select` a mais, estreitar depois que alguém leu a correspondência alheia é impossível. O vínculo sai de um **JOIN no endereço**, nunca gravado — e **endereço da casa nunca vira chave**, senão quem edita o cadastro do cliente escolheria a chave da consulta e leria a caixa de um colega. Ficam fora sempre: marcados como particular, Lixeira/Spam/**Rascunhos** e as cópias repetidas (dedup por `messageId`). A **válvula** é do dono e **tem volta**: a ficha só tira, devolver é na caixa. Caixa fora do ar não derruba o card — mostra o log automático e avisa. **Faltam 2D‑2** (anexo vira documento do cliente) **e 2D‑3** (e-mail de desconhecido vira lead).
...
- **Captação pública + Portal do prospect (ADR-15):** form `/comecar` (honeypot + rate-limit) com **detecção de origem** (UTM/referrer/ads e cadastro manual rastreado); recaptura deduplica. Acesso automático ao Portal (`garantirAcessoPortal`) — o prospect acompanha o atendimento (etapa traduzida, documentos para assinar) desde o 1º contato; continuidade na conversão.
