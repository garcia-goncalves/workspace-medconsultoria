...
**Evolução pós-MVP (entregue):** **e-mail dentro da aplicação** — Bloco 1 (plugar e ler, ADR-95), Bloco 2 fase 2A (escrever/responder/encaminhar/anexar, ADR-96) e fase 2D‑1 (a conversa com o cliente na ficha, ADR-97); faltam 2D‑2 (anexo vira documento) e 2D‑3 (e-mail de desconhecido vira lead), funil inteligente (playbook + serviços + passos automáticos + geração de documentos), captação pública + Portal do prospect + acesso automático, sistema de e-mails branded (SMTP + templates editáveis + histórico + preferências), assinatura eletrônica de documentos, painel Sistema/observabilidade (ROOT), dashboard por papel, e o pacote de funil ganho/perda (lead perdido reversível + taxa de conversão + conversão→Financeiro/Agenda + origem comercial na ficha).
