...
- **E-mail dentro da aplicação (ADR-95/96):** cada pessoa **pluga a própria caixa** `@medconsultoria.com.br` em `/email` (a mesma senha do webmail, testada antes de gravar e guardada cifrada) e lê/**escreve/responde/encaminha/anexa** sem sair do Workspace. **A caixa é privada, a correspondência com o cliente é da empresa:** ninguém abre a caixa de ninguém (todo procedure filtra por `userId`), mas e-mail trocado com cliente/lead cadastrado aparece na ficha dele — resolvido por JOIN no endereço, nunca gravado fixo — e o dono pode **marcar como particular** para tirá-lo de lá. Responder marca a original como respondida (`\Answered` no servidor); encaminhar não (não é resposta a quem escreveu). Enquanto escreve, o rascunho grava sozinho na pasta **`Drafts` do servidor** (aparece no webmail). Fora de produção, só é permitido enviar para os dois e-mails de teste do dono (`conferirDestinoPermitido`) — o SMTP é real em qualquer ambiente. **Não existe caixa compartilhada gerida pela app:** quem manda no acesso é o servidor de e-mail (cortar = trocar a senha na hospedagem). Fase 1 só o domínio da Med; Gmail/Outlook exigem OAuth e ficaram para depois. Falta o bloco de e-mails na ficha do cliente. Não confundir `/email` (a caixa) com `/emails` (mensagens automáticas) nem `/emails-enviados` (monitor de envios).
...
**Evolução pós-MVP (entregue):** **e-mail dentro da aplicação — Bloco 1: plugar e ler** (caixa IMAP por usuário em `/email`, ADR-95; falta o Bloco 2 = responder/escrever + e-mails na ficha), funil inteligente (playbook + serviços + passos automáticos + geração de documentos), captação pública + Portal do prospect + acesso automático, sistema de e-mails branded (SMTP + templates editáveis + histórico + preferências), assinatura eletrônica de documentos, painel Sistema/observabilidade (ROOT), dashboard por papel, e o pacote de funil ganho/perda (lead perdido reversível + taxa de conversão + conversão→Financeiro/Agenda + origem comercial na ficha).
