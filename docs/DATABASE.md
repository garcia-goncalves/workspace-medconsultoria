...
### Servico
Catálogo de serviços da MedConsultoria (editável). **⚠️ `@@unique([nome])` desde a ADR-147** (migração `20260902000000`, aditiva; reverter é `DROP INDEX`): o nome IDENTIFICA o serviço, porque a semeadura casa por nome e a proposta lista dois iguais sem distingui-los. ⚠️ **A coluna é `utf8mb4_unicode_ci` — o banco ignora MAIÚSCULA e ACENTO**, então `"Conteudo & SEO"` e `"Conteúdo & SEO"` são a MESMA linha para o índice. Toda comparação de nome na aplicação usa a mesma régua por `chaveDoNomeDeServico` (`apps/api/src/modules/servicos/chave-de-nome.ts`); comparar com igualdade crua de JavaScript fazia a semeadura tentar recriar um canônico existente e derrubar a rota PÚBLICA com `P2002`. `id`, `nome`, `descricao?`, **`categoria?`** (pilar: Gestão|Faturamento|Networking|Desenvolvimento|Marketing — ADR-27), `ativo`, `ordem`, `createdAt`. **Precificação flexível (ADR-33):** **`valor? Decimal @db.Decimal(12,2)`** (valor fixo de referência — era `Float` até a ADR-118) + **`valorRecorrencia PrecoRecorrencia @default(AVULSO)`**; **`percentual? Decimal @db.Decimal(12,2)`** (% sobre o faturamento do cliente, ex.: 5 = 5%) + **`percentualRecorrencia PrecoRecorrencia @default(MENSAL)`** — enum **`PrecoRecorrencia { AVULSO, MENSAL }`**. Um serviço pode ter valor fixo e/ou %, cada um avulso ou mensal. **⚠️ Desde a ADR-145, só UM serviço pode ter percentual: o marcado por `ehFaturamento Boolean @default(false)`** (migração `20260901010000`, aditiva, backfill por PREÇO — `percentual > 0 AND (valor IS NULL OR valor = 0)`). Ordem do dono: a Med só recebe percentual no faturamento médico; todo o resto é valor fixo, avulso ou mensal. A marca é lida por `ehServicoDeFaturamento` (`@app/shared`) e travada em quatro portas (Zod da criação, servidor na criação, servidor na edição sobre ANTES+DEPOIS, e a ficha do cliente); há guarda de **marca única** e de **nunca faturamento + credenciamento no mesmo serviço**. ⚠️ **Não confundir com `ehServicoSomentePercentual`**, que lê o PREÇO e responde "como esta linha está cobrada" — essa não mudou. **Na UI o % só aparece no serviço marcado (ADR-125/145)** — antes a checagem era por categoria, e ela fazia abrir-e-salvar outro serviço apagar o percentual dele em silêncio. Também **`ehCredenciamento Boolean @default(false)` (ADR-144)**. Também **`condicaoPagamento? @Text` (ADR-125)**: a frase de condição de pagamento que a PROPOSTA pré-preenche quando este serviço é escolhido (o Faturamento nasce com "O recebimento do Repasse será sempre feito após o crédito na conta da Clínica."); mesmo molde de `clausulasContrato` — editável em Serviços, nunca escrita no código. Relações: `leads[]` (LeadServicos, N-N), `passos[]`, `requisitos[]`, `contratacoes[]` (ClienteServico), `arquivos[]`. O lead marca o que precisa; guia o checklist do funil e vira tarefas do projeto na conversão. Catálogo granular real (10 serviços) semeado a partir de `brand/` + site.
...
`id`, `tipo (ContaTipo: PAGAR|RECEBER)`, **`escopo (Escopo: EMPRESA|PESSOAL)`**, **`donoId?`** (dono da carteira PESSOAL; null em EMPRESA), `descricao`, `valor Decimal(12,2)`, `vencimento`, `pago`, `pagoEm?`, `categoriaId?`, `clienteId?`, `recorrencia (Recorrencia)`, **`recorrenciaAte?`** (limite da série), **`recorrenteId?`** (âncora = id da 1ª conta da série; agrupa as geradas), **`origemServicoId?`** (ADR-148 — de que serviço veio a cobrança; existe porque a conferência que impede cobrar duas vezes o mesmo serviço casava por TEXTO da descrição `"<Serviço> — <Cliente>"`, e renomear o cliente fazia ela deixar de casar com as cobranças antigas, lançando tudo de novo em silêncio. ⚠️ A conferência olha as **duas** coisas: o id cobre as contas novas, o texto cobre as anteriores à coluna — não houve backfill, porque deduzir o serviço da descrição é a fragilidade que a coluna veio substituir), `observacoes?`, timestamps, `deletedAt?`. Índices: `[escopo,donoId]`, `[recorrenteId]`, `[clienteId,origemServicoId]`, **`@@unique([recorrenteId, vencimento])`** (ADR-93: uma série não pode ter duas parcelas no mesmo vencimento — vale inclusive para linhas soft-deletadas, por isso a geração ressuscita a apagada em vez de criar outra; conta não-recorrente tem `recorrenteId` nulo e fica fora, pois o MySQL trata NULL como distinto). **Carteiras (ADR-45):** EMPRESA = livros da Med (compartilhada ADMIN/ROOT); PESSOAL = privada por usuário (`donoId`). **Recorrência materializa** (ADR-45): marcar paga cria a próxima ocorrência (mesma série via `recorrenteId`). Contas a receber ainda são **provisionadas na conversão de um lead** (EMPRESA, a partir do `valorEstimado`).
...
### Documento
`id`, `modeloId?`, `clienteId?`, `projetoId?`, `titulo`, `conteudo @Text` (Markdown), `status (StatusDocumento: RASCUNHO|EM_REVISAO|APROVADO|ENVIADO)`, `criadoPorId`, `aprovadoPorId?`, `enviadoEm?`, **`assinaturaSolicitadaEm? / assinadoEm?`**, **aceite online da proposta (ADR-47): `propostaToken? @unique`, `propostaStatus?` (PENDENTE|ACEITA|RECUSADA), `propostaHash?`, `propostaSolicitadaEm? / propostaRespondidaEm? / propostaRespIp? / propostaMotivoRecusa?`**, **`propostaRespPorId?`** (ADR-137 — QUEM aceitou, quando a pessoa estava logada; nulo é o caso normal, porque o link de e-mail é anônimo), timestamps, `deletedAt?`. Relações: `versoes[]`, `assinaturas[]`, `credenciamentos[]`.
...
### Assinatura
Assinatura eletrônica avançada (Lei 14.063/2020) por signatário. `id`, `documentoId`, `papel` ("CLIENTE"|"MEDCONSULTORIA"), `nome`, `email?`, `ordem`, **`token @unique`** (link de assinatura), `status` (PENDENTE|ASSINADO), `metodo?` (DESENHO|DIGITADO), `imagem? @Text` (data-URI do traço), `nomeDigitado?`, **`hashDocumento?`** (sha256 do conteúdo no envio — prova de integridade), `ip?`, `userAgent?`, **`assinadoPorId?`** (ADR-137 — QUEM clicou, quando a pessoa estava logada; IP e user-agent dizem de onde veio, nunca quem foi), `assinadoEm?`, **`consentimentoEm?`** e **`consentimentoVersao?`** (ADR-148 — a prova do "li o documento e concordo". A caixa sempre foi exigida pela tela e pelo servidor e **nada era gravado**: passado o clique não sobrava registro de que a pessoa consentiu nem com que texto. ⚠️ É data **mais** versão, porque o texto muda e só a data não diria com o quê; o texto vive em `packages/shared/src/consentimento-assinatura.ts`, com teste que reprova quem editá-lo sem subir a versão. ⚠️ Nulo nas assinaturas anteriores à coluna, e a tela diz "não registrado" — preencher com a data da assinatura fabricaria uma prova que ninguém coletou), `criadoEm`. Trilha de auditoria completa. ⚠️ **O `token` NÃO volta mais no painel interno do documento** (ADR-148): ele é a chave de uma rota pública que assina, e vinha em claro para qualquer FUNCIONARIO — como o signatário legítimo também assina deslogado (`assinadoPorId: null`), uma assinatura fabricada ficava indistinguível da verdadeira. O link sai por `/ir/assinar/:id`, que exige sessão e registra quem abriu.
