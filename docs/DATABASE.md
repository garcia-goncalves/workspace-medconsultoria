...
### Operadora
Catálogo editável de operadoras/convênios para a **Proposta de credenciamento** (ADR-58). `id`, `nome`, `ordem`, `createdAt`. Semeado com `OPERADORAS_COMUNS` na 1ª leitura; a equipe renomeia e exclui pela tela. Router `documentos.operadoras` (list/criar/renomear/remover). **Desde o ADR-104 há FK:** `Credenciamento.operadoraId` é `Restrict`, então **operadora com credenciamento registrado não pode ser excluída** — apagá-la levaria junto o andamento e o elo com a cobrança. O serviço confere antes e devolve um recado dizendo quantos processos estão presos nela; renomear continua livre.
