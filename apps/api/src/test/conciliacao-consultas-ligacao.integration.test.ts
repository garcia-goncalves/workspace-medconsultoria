...
const { ligarProfissional } = await import("../modules/conciliacao/conciliacao.service.js");
const { pendenciasDePara } = await import("../modules/conciliacao/conciliacao-painel.service.js");
...
describe("#3 — ligar profissional", () => {
  it("cliente sem médico cadastrado: a pendência existe e não há a quem ligar", async () => {
    ...
    expect(p.profissionais[0]!.atendimentos).toBe(3);
  ...
  it("depois de cadastrar e ligar, retroage em consultas E cirurgias, de qualquer caixa", async () => {
    ...
    expect(r.linhasAtualizadas).toBe(3);
